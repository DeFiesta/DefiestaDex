...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const factory = await deployments.get("DefiestaDexFactory");
  const router = await deployments.get("DefiestaDexRouter");
  ...
  if (list.length > 0) {
    ...
    for (let i = 0; i < tokenList.length; i++) {
      ...
      if (!isMainnet && (isTether || token.artifact === "TestToken")) {
        ...
        if (token.artifact !== "") {
          ...
          tokenInstance = await getContractFactory("DefiestaDexTokenTest");
          token.artifact = "DefiestaDexTokenTest";
        }
    ...
    const factoryInstance = await getContractAt("DefiestaDexFactory", factory.address);
    const routerInstance = await getContractAt("DefiestaDexRouter", router.address);
    ...
    for (let i = 0; i < pairs.length; i++) {
      ...
      const pairFactory = await getContractFactory("DefiestaDexPair");
