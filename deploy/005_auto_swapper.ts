...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const DDEX = await deployments.get("DefiestaDexToken");

  let staking: boolean = true;
  let rewardManagerArtifact: string = "RewardManager";
  let stakingAddress: string = "";
  const args: string[] = [factory.address, DDEX.address];
  ...
  if (chainId !== "1") {
    const factoryContract = await getContractAt("DefiestaDexFactory", factory.address);
