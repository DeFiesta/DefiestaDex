...
export function shouldBehaveLikeRewardManager() {
  ...
  let DefiestaDexToken: ERC20Permit;
  let startBlock: BigNumber;
  let tether: TetherToken;

  beforeEach(async function () {
    ({ admin } = this.signers);
    ({ rewardManagerTest, DefiestaDexToken, tether } = this.contracts);
  ...
  describe("Constructor and init", async function () {
    ...
    it("should not deploy when start farming parameter is before current block", async function () {
      ...
      await expect(factory.deploy(admin.address, DefiestaDexToken.address, 0, { gasLimit: 10000000 })).to.be.revertedWith(
    ...
    context("When reward Manager is a contract", function () {
      it("should be able to add reward info and approve reward tokens on reward Manager", async function () {
        const farmingAddress = await rewardManagerTest.farming();
        const farmingContract = FarmingRange__factory.connect(farmingAddress, admin);
        await DefiestaDexToken.transfer(rewardManagerTest.address, parseEther("10000"));

        const balance = await DefiestaDexToken.balanceOf(rewardManagerTest.address);

        expect((await farmingContract.campaignInfo(0)).rewardToken).to.eq(DefiestaDexToken.address);

        // add the first reward info
        expect(await DefiestaDexToken.allowance(rewardManagerTest.address, farmingContract.address)).to.eq(0);
        ...
        expect(await DefiestaDexToken.allowance(rewardManagerTest.address, farmingContract.address)).to.eq(
        ...
        const balance_after = await DefiestaDexToken.balanceOf(rewardManagerTest.address);
      ...
      it("should be able to call resetAllowance for the token rewarded in a campaign, with allowance not 0 and set the allowance to max", async function () {
        ...
        await rewardManagerTest.setAllowance(DefiestaDexToken.address, 10);
        expect(await DefiestaDexToken.allowance(rewardManagerTest.address, farmingContract.address)).to.eq(10);
