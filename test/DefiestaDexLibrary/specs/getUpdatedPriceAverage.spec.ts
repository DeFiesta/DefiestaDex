...
export function shouldBehaveLikeGetUpdatedPriceAverage(): void {
  ...
  it("first timestamp null", async function () {
    const [priceAvIn, priceAvOut] = await this.contracts.DefiestaDexLibraryTest.getUpdatedPriceAverage(
      RES_FIC_IN,
      RES_FIC_OUT,
      "0",
      OLD_PRICE_AV_IN,
    ...
    const [priceAvInReversed, priceAvOutReversed] = await this.contracts.DefiestaDexLibraryTest.getUpdatedPriceAverage(
      RES_FIC_IN,
      RES_FIC_OUT,
      0,
      OLD_PRICE_AV_IN,
  ...
  it("first timestamp below current timestamp", async function () {
    await expect(
      this.contracts.DefiestaDexLibraryTest.getUpdatedPriceAverage(
        RES_FIC_IN,
        RES_FIC_OUT,
        TIMESTAMP_2023 + 1,
        OLD_PRICE_AV_IN,
    ...
    ).to.be.revertedWith("DefiestaDexPair: INVALID_TIMESTAMP");
  });
  ...
  it("timeDiff = 12, One For Zero", async function () {
    const TIME_DIFF = 12;
    const [priceAvOut, priceAvIn] = await this.contracts.DefiestaDexLibraryTest.getUpdatedPriceAverage(
      RES_FIC_OUT,
      RES_FIC_IN,
      TIMESTAMP_2023,
      OLD_PRICE_AV_OUT,
