...
export function shouldBehaveLikeGetAmountOut(): void {
  it(`should revert when INSUFFICIENT_LIQUIDITY`, async function () {
    await expect(this.contracts.DefiestaDexRouter.getAmountOut(amountsOutParams[0])).to.be.revertedWith(
      "DefiestaDexLibrary: INSUFFICIENT_LIQUIDITY",
    );
    await expect(this.contracts.DefiestaDexRouter.getAmountOut(amountsOutParams[0])).to.be.revertedWith(
      "DefiestaDexLibrary: INSUFFICIENT_LIQUIDITY",
    );
    await expect(this.contracts.DefiestaDexRouter.getAmountOut(amountsOutParams[1])).to.be.revertedWith(
      "DefiestaDexLibrary: INSUFFICIENT_LIQUIDITY",
    );
  });

  it(`should revert when INSUFFICIENT_INPUT_AMOUNT`, async function () {
    await expect(this.contracts.DefiestaDexRouter.getAmountOut(amountsOutParams[2])).to.be.revertedWith(
      "DefiestaDexLibrary: INSUFFICIENT_INPUT_AMOUNT",
    );
  });

  it(`should return zero amount out with low parameters `, async function () {
    expect((await this.contracts.DefiestaDexRouter.getAmountOut(amountsOutParams[3])).amountOut_).to.eq(0);
  });
  ...
  getAmountSimpleTestData.concat(getAmount2TradesTestData).forEach((getAmountTestCase, i) => {
    it(`getAmountOut simple check : ${i + 1}`, async function () {
      ...
      const result = await this.contracts.DefiestaDexLibraryTest.getAmountOut(args);
  ...
  getAmount2TradesTestData.forEach((getAmountTestCase, i) => {
    it(`getAmountOut check 2 trades : ${i + 1}`, async function () {
      ...
      const paramFirstAmount = await this.contracts.DefiestaDexLibraryTest.computeFirstTradeQtyIn(
        paramAmount,
      ...
      const resultApplyK = await this.contracts.DefiestaDexLibraryTest.applyKConstRuleOut(
      ...
      const computeFictiveReservesResult = await this.contracts.DefiestaDexLibraryTest.computeFictiveReserves(
      ...
      const secondResultApplyK = await this.contracts.DefiestaDexLibraryTest.applyKConstRuleOut(
