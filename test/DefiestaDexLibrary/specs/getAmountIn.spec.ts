...
export function shouldBehaveLikeGetAmountIn(): void {
  it("should revert when INSUFFICIENT_LIQUIDITY", async function () {
    await expect(this.contracts.DefiestaDexRouter.getAmountIn(amountsInParams[0])).to.be.revertedWith(
      "DefiestaDexLibrary: INSUFFICIENT_LIQUIDITY",
    );

    await expect(this.contracts.DefiestaDexRouter.getAmountIn(amountsInParams[1])).to.be.revertedWith(
      "DefiestaDexLibrary: INSUFFICIENT_LIQUIDITY",
    );
  });

  it(`should revert when INSUFFICIENT_OUTPUT_AMOUNT`, async function () {
    await expect(this.contracts.DefiestaDexRouter.getAmountIn(amountsInParams[2])).to.be.revertedWith(
      "DefiestaDexLibrary: INSUFFICIENT_OUTPUT_AMOUNT",
    );
  });

  it(`execute with low parameters `, async function () {
    expect((await this.contracts.DefiestaDexRouter.getAmountIn(amountsInParams[3])).amountIn_).to.eq(2);
  });
  ...
  getAmountSimpleTestData.concat(getAmount2TradesTestData).forEach((getAmountTestCase, i) => {
    it(`getAmountIn simple check : ${i + 1}`, async function () {
      ...
      const result = await this.contracts.DefiestaDexLibraryTest.getAmountIn(args);
  ...
  getAmount2TradesTestData.forEach((getAmountTestCase, i) => {
    it(`getAmountIn check 2 trades : ${i + 1}`, async function () {
      ...
      const paramFirstAmount = await this.contracts.DefiestaDexLibraryTest.computeFirstTradeQtyOut(
        paramAmount,
      ...
      const resultApplyK = await this.contracts.DefiestaDexLibraryTest.applyKConstRuleIn(
        paramFirstAmount,
      ...
      const secondResultApplyK = await this.contracts.DefiestaDexLibraryTest.applyKConstRuleIn(
        paramSecondAmount,
        resultApplyK.newReserveIn_,
        resultApplyK.newReserveOut_,
