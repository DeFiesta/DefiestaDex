...
export function shouldBehaveLikeRemoveLiquidityETH(): void {
  it("removeLiquidityETH", async function () {
    ...
    await this.contracts.WETHPair.approve(this.contracts.DefiestaDexRouter.address, constants.MaxUint256);
    ...
    await expect(
      this.contracts.DefiestaDexRouter.removeLiquidityETH(
      ...
      .withArgs(this.contracts.WETHPair.address, this.contracts.DefiestaDexRouter.address, ETHAmount.sub(2000))
      .to.emit(this.contracts.WETHPartner, "Transfer")
      .withArgs(this.contracts.WETHPair.address, this.contracts.DefiestaDexRouter.address, WETHPartnerAmount.sub(500))
      .to.emit(this.contracts.WETHPartner, "Transfer")
      .withArgs(this.contracts.DefiestaDexRouter.address, this.signers.admin.address, WETHPartnerAmount.sub(500))
