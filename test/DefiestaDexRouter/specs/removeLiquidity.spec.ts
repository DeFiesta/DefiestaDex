...
export function shouldBehaveLikeRemoveLiquidity(): void {
  ...
  it("simple test", async function () {
    ...
    await expect(
      this.contracts.DefiestaDexRouter.removeLiquidity(
      ...
      .to.emit(this.contracts.DefiestaDexPair, "Transfer")
      .withArgs(this.contracts.DefiestaDexPair.address, constants.AddressZero, expectedLiquidity.sub(MINIMUM_LIQUIDITY))
      .to.emit(this.contracts.token0, "Transfer")
      .withArgs(this.contracts.DefiestaDexPair.address, this.signers.admin.address, amountToReceiveToken0)
      .to.emit(this.contracts.token1, "Transfer")
      .withArgs(this.contracts.DefiestaDexPair.address, this.signers.admin.address, amountToReceiveToken1)
      .to.emit(this.contracts.DefiestaDexPair, "Sync")
      .withArgs(500, 2000, anyValue, anyValue, 0, 0)
      .to.emit(this.contracts.DefiestaDexPair, "Burn")
      .withArgs(
    ...
    expect(await this.contracts.DefiestaDexPair.balanceOf(this.signers.admin.address)).to.eq(0);
  ...
  it("trying to set a fictive reserve to 0 by burn", async function () {
    ...
    const userLP = await this.contracts.DefiestaDexPair.balanceOf(this.signers.admin.address);
    ...
    ).to.not.be.revertedWith("DefiestaDex: FICTIVE_RESERVES_TOO_LOW");

    const ficRes = await this.contracts.DefiestaDexPair.getFictiveReserves();
