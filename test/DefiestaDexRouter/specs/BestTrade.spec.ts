...
import { unitFixtureDefiestaDexRouter, unitFixtureTokensAndPairWithFactory } from "../../fixtures";
import hre from "hardhat";
import { ERC20Test, DefiestaDexPair, DefiestaDexPair__factory, DefiestaDexRouter } from "../../../typechain";
...
async function make_a_trade(
  ...
  contracts: Contracts | { DefiestaDexRouter: DefiestaDexRouter; token0: ERC20Test; token1: ERC20Test },
  autoMine = true,
) {
  ...
  if (amount0.lt(0)) {
    // user buy token0
    receipt = await contracts.DefiestaDexRouter.connect(signer).swapExactTokensForTokens(
      amount0.abs(),
      1,
    ...
    receipt = await contracts.DefiestaDexRouter.connect(signer).swapTokensForExactTokens(
      amount0.abs(),
      constants.MaxUint256,
...
async function show_reserves(contracts: Contracts | { DefiestaDexPair: DefiestaDexPair }) {
  if (!LOGGING) return;
  const reserves = await contracts.DefiestaDexPair.getReserves();
  const fictiveReserves = await contracts.DefiestaDexPair.getFictiveReserves();
...
export function checkBestTrade() {
  describe("Worst cases", function () {
    it("return the worst new K", async function () {
      ...
      let pairToken0Balance = await this.contracts.token0.balanceOf(this.contracts.DefiestaDexPair.address);
      let pairToken1Balance = await this.contracts.token1.balanceOf(this.contracts.DefiestaDexPair.address);
      let fictiveReserves = await this.contracts.DefiestaDexPair.getFictiveReserves();

      //swap 1
      await this.contracts.token0.transfer(this.contracts.DefiestaDexPair.address, parseEther("5"));
      ...
      pairToken0Balance = await this.contracts.token0.balanceOf(this.contracts.DefiestaDexPair.address);
      pairToken1Balance = await this.contracts.token1.balanceOf(this.contracts.DefiestaDexPair.address);
      fictiveReserves = await this.contracts.DefiestaDexPair.getFictiveReserves();

      await this.contracts.token1.transfer(this.contracts.DefiestaDexPair.address, parseEther("45"));
      await this.contracts.DefiestaDexRouter.swapExactTokensForTokens(
        parseEther("45"),
        1,
    ...
    it("swap loops in and out and check output values increasing for LPs", async function () {
       ...
       */
      await this.contracts.DefiestaDexRouter.connect(this.signers.user).swapTokensForExactTokens(
        parseEther("5"),
        constants.MaxUint256,
       ...
       * ____________________________________________________
       */
      await this.contracts.DefiestaDexRouter.connect(this.signers.user).swapExactTokensForTokens(
...
export function testExploit() {
  ...
  before(async function () {
    ...
    const { DefiestaDexRouter, factory } = await unitFixtureDefiestaDexRouter();
    for (let indexSwap = 1; indexSwap < 50; indexSwap++) {
      const { token0, token1 } = await unitFixtureTokensAndPairWithFactory(factory);

      await token0.approve(DefiestaDexRouter.address, constants.MaxUint256);
      await token1.approve(DefiestaDexRouter.address, constants.MaxUint256);

      //Approve a 2 steps transfer to the contract (user)
      await token0.connect(signers[1]).approve(DefiestaDexRouter.address, constants.MaxUint256);
      await token1.connect(signers[1]).approve(DefiestaDexRouter.address, constants.MaxUint256);
      ...
      await DefiestaDexRouter.connect(signers[0]).addLiquidity(
        token0.address, //ETH
        token1.address, //USDT
      ...
      await make_a_trade(parseEther("2"), signers[0], { DefiestaDexRouter, token0, token1 });
      await make_a_trade(parseEther("0.01"), signers[0], { DefiestaDexRouter, token0, token1 });
      ...
      try {
        await make_a_trade(
          ...
          signers[1],
          { DefiestaDexRouter, token0, token1 },
          false, //avoid mining to have both swaps in same tx
        );
        await make_a_trade(parseEther(indexSwap.toString()), signers[1], { DefiestaDexRouter, token0, token1 });
      } catch (_error) {
        error = true;
      }
...
// TEST CASE 2
export function testNewDefiestaDex() {
  let bestGain = constants.Zero;
  before(async function () {
    const signers = await hre.ethers.getSigners();
    const { DefiestaDexRouter, factory } = await unitFixtureDefiestaDexRouter();
    ...
    for (let indexSwap = 1; indexSwap < 50; indexSwap++) {
      ...
      await token0.connect(signers[0]).approve(DefiestaDexRouter.address, constants.MaxUint256);
      await token1.connect(signers[0]).approve(DefiestaDexRouter.address, constants.MaxUint256);
      ...
      let error = false;
      try {
        await make_a_trade(parseEther("0.1"), signers[0], { DefiestaDexRouter, token0, token1 });
        await make_a_trade(parseEther("2"), signers[0], { DefiestaDexRouter, token0, token1 });
        await make_a_trade(parseEther("-2.5"), signers[0], { DefiestaDexRouter, token0, token1 });
        await make_a_trade(parseEther("5"), signers[0], { DefiestaDexRouter, token0, token1 });

        await doubleSwapRouter
          .connect(signers[1])
          .doubleSwapExactOutExactIn(DefiestaDexRouter.address, indexSwap, parseEther("20000000"), [
            token1.address,
            token0.address,
          ]);
      } catch (_error) {
...
export function testHack4() {
  ...
  it("Should check bestGain after loops v4", async function () {
    ...
    const DefiestaDexPairContract = DefiestaDexPair__factory.connect(
    ...
    await show_reserves({
      DefiestaDexPair: DefiestaDexPairContract,
    });
    ...
    await make_a_trade(parseEther("3"), signers[0], { DefiestaDexRouter, token0, token1 });
    ...
    await expect(
      doubleSwapRouter
        .connect(signers[1])
        .doubleSwapExactOutExactIn(DefiestaDexRouter.address, parseEther("4.1"), parseEther("20000000"), [
          token1.address,
          token0.address,
        ]),
    ).to.be.revertedWith("DefiestaDexLibrary: INSUFFICIENT_LIQUIDITY");
    await doubleSwapRouter
      .connect(signers[1])
      .doubleSwapExactOutExactIn(DefiestaDexRouter.address, parseEther("3.9"), parseEther("20000000"), [
        token1.address,
        token0.address,
      ]);
    const balance1_after = await token1.balanceOf(signers[1].address);
