...
import { deployERC20Test, deployDefiestaDexPairTest } from "../../deployers";
import { parseEther } from "ethers/lib/utils";
import { GetAmountTrade } from "../../DefiestaDexLibrary/utils";
import { getImpersonatedSigner } from "@nomiclabs/hardhat-ethers/internal/helpers";
import hre, { ethers } from "hardhat";
import {
  DefiestaDexFactory__factory,
  DefiestaDexPair__factory,
  DefiestaDexRouter__factory,
  DefiestaDexToken__factory,
} from "../../../typechain";
...
export function shouldBehaveLikeCheckFailedTest(): void {
  it("try revert case", async function () {
    ...
    this.contracts.DefiestaDexPairTest = await deployDefiestaDexPairTest(
      this.contracts.DefiestaDexFactoryTest,
      this.contracts.WETH,
      WETHPartner,
    );
    const token0InPair = await this.contracts.DefiestaDexPairTest.token0();
  ...
  it.skip("try revert case at a fixed block", async function () {
    ...
    const routerContract = DefiestaDexRouter__factory.connect(
  ...
  it.skip("check fees on snap", async function () {
    ...
    const tokenDDEX = DefiestaDexToken__factory.connect("0x3B20f0064eb855eECC7f3f7Da58c282C38e02607", impersonatedSigner);
    const tokenUSDT = DefiestaDexToken__factory.connect("0xEc1e8a6cE865C50f956ca922C3DE8F3242B2c17B", impersonatedSigner);
    const routerContract = DefiestaDexRouter__factory.connect(
      "0x3c7f9da73afe76f70324abba7fe924d5c8e81308",
      impersonatedSigner,
    );
    const factoryContract = DefiestaDexFactory__factory.connect(await routerContract.factory(), impersonatedSigner);
    const pairAddress = await factoryContract.getPair(tokenDDEX.address, tokenUSDT.address);
    console.log(pairAddress);
    const pair = DefiestaDexPair__factory.connect(pairAddress, impersonatedSigner);

    // const amountUSDT = parseUnits("100000", 6);
    const amountDefiestaDex = parseEther("100000");
    ...
    let result = await routerContract
      ...
      .swapTokensForExactTokens(
        ...
        26226898441,
        [tokenUSDT.address, tokenDDEX.address],
        impersonatedSigner.address,
        1677850602,
        {
          gasLimit: 139898,
    ...
    result = await routerContract
      ...
      .swapExactTokensForTokens(
        "100000000000000000000000",
        26151072198,
        [tokenDDEX.address, tokenUSDT.address],
        impersonatedSigner.address,
        1677850636,
        {
          gasLimit: 136517,
    ...
    //token0 = DDEX
    expect(feesAfter.fees0_).to.be.eq(feesBefore.fees0_.add(amountDefiestaDex.mul(FEES_POOL).div(FEES_BASE)));
  ...
  async function setupPair(
    ...
    await contracts.routerForPairTest.mint(
      contracts.DefiestaDexPairTest.address,
    ...
    const reserves = await contracts.DefiestaDexPairTest.getReserves();
    ...
    await contracts.DefiestaDexPairTest.setFictivePoolValues(
    ...
    await contracts.DefiestaDexPairTest.setPriceAverage(
