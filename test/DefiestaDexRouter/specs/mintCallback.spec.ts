...
export function shouldBehaveLikeMintCallback() {
  ...
  it("should addLiquidity with MintCallback and fail with missing tokens", async function () {
    ...
    // get fees
    const feeToAmount = await this.contracts.DefiestaDexPair.getFeeToAmounts();
    expect(feeToAmount.fees0_).to.be.gt(0);
    expect(feeToAmount.fees1_).to.be.gt(0);
    await expect(
      this.contracts.DefiestaDexRouterCallbackTest.addLiquidity(
        this.contracts.token0.address,
        this.contracts.token1.address,
    ...
    ).to.be.revertedWith("DefiestaDex: INSUFFICIENT_AMOUNT_1");

    await this.contracts.DefiestaDexRouterCallbackTest.setIsToken0ToLower(true);
    ...
    ).to.be.revertedWith("DefiestaDex: INSUFFICIENT_AMOUNT_0");
    expect(await this.contracts.DefiestaDexPair.balanceOf(this.signers.admin.address)).to.be.gt(0);
  });
  it("should addLiquidity with MintCallback and fail reentrancy", async function () {
    await this.contracts.fakeERC20reentrancy.approve(this.contracts.DefiestaDexRouter.address, constants.MaxUint256);
    ...
    ).to.be.revertedWith("DefiestaDex: LOCKED");
  });
  ...
  it("mint callback should fail when not called by pair", async function () {
    ...
    await expect(this.contracts.DefiestaDexRouter.DefiestaDexMintCallback(data)).to.be.revertedWith(
      "DefiestaDexRouter: INVALID_PAIR",
    );
  });
  ...
  it("mint callback should fail when no amount", async function () {
    ...
    await expect(this.contracts.fakeERC20reentrancy.DefiestaDexMintCallback(data)).to.be.revertedWith(
