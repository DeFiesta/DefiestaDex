...
export function shouldBehaveLikeSwapCallback() {
  it("swap callback should fail when no amount in parameters", async function () {
    await expect(this.contracts.DefiestaDexRouter.DefiestaDexSwapCallback(0, 0, constants.HashZero)).to.be.revertedWith(
  ...
  it("swap callback should fail when not called by pair", async function () {
    await expect(
      this.contracts.DefiestaDexRouter.DefiestaDexSwapCallback(
        10000,
        100000,
        getSwapEncodedData(
    ...
    ).to.be.revertedWith("DefiestaDexRouter: INVALID_PAIR");
  });
}
