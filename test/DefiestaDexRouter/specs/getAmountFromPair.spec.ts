...
import { getAmount2TradesTestData } from "../../DefiestaDexLibrary/utils";
import { ethers } from "hardhat";

export function shouldBehaveLikeDefiestaDexRouterGetAmountFromPair(): void {
  getAmount2TradesTestData.forEach((getAmountTestCase, i) => {
    it(`getAmountOut simple check : ${i + 1}`, async function () {
      await this.contracts.token0.transfer(this.contracts.DefiestaDexPairTest.address, getAmountTestCase.reserveToken0);
      await this.contracts.token1.transfer(this.contracts.DefiestaDexPairTest.address, getAmountTestCase.reserveToken1);

      await this.contracts.DefiestaDexPairTest.setFictivePoolValues(
      ...
      await this.contracts.DefiestaDexPairTest.setPriceAverage(
        getAmountTestCase.priceAverageToken0,
        getAmountTestCase.priceAverageToken1,
        timestamp,
      );

      const result = await this.contracts.DefiestaDexRouterTest.getAmountOutFromPair(
  ...
  getAmount2TradesTestData.forEach((getAmountTestCase, i) => {
    it(`getAmountOut simple check : ${i + 1}`, async function () {
      ...
      const result = await this.contracts.DefiestaDexRouterTest.getAmountInFromPair(
        getAmountTestCase.amountOutToken1,
        await this.contracts.token0.address,
        await this.contracts.token1.address,
      );
