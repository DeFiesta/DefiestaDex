...
import { unitFixtureDefiestaDexRouter } from "../../fixtures";
...
export function shouldBehaveLikeAddLiquidity(): void {
  it("simple test", async function () {
    ...
    await expect(
      ...
      .withArgs(this.signers.admin.address, this.contracts.DefiestaDexPair.address, token0Amount)
      .to.emit(this.contracts.token1, "Transfer")
      .withArgs(this.signers.admin.address, this.contracts.DefiestaDexPair.address, token1Amount)
      ...
      .to.emit(this.contracts.DefiestaDexPair, "Mint")
      .withArgs(this.contracts.DefiestaDexRouter.address, this.signers.admin.address, token0Amount, token1Amount);

    expect(await this.contracts.DefiestaDexPair.balanceOf(this.signers.admin.address)).to.eq(
  ...
  it("should fail to create pair with same token twice", async function () {
    ...
    ).to.be.revertedWith("DefiestaDex: IDENTICAL_ADDRESSES");
  });
  ...
  it("should fail to create pair with zero Address", async function () {
    ...
    ).to.be.revertedWith("DefiestaDex: ZERO_ADDRESS");
  });
  ...
  it("should fail with Insufficient Liquidity Minted", async function () {
    ...
    ).to.be.revertedWith("DefiestaDex: INSUFFICIENT_LIQUIDITY_MINTED");
  });
  ...
  it("should fail when deadline is before current timestamp", async function () {
    ...
    ).to.be.revertedWith("DefiestaDexRouter: EXPIRED");
  });
  ...
  it("addLiquidity after swap and cover path for amounts Optimal", async function () {
    ...
    const reserves = await this.contracts.DefiestaDexPair.getReserves();

    const quoteB = await this.contracts.DefiestaDexRouter.quote(token0Amount, reserves.reserve0_, reserves.reserve1_);
    ...
    ).to.be.revertedWith("DefiestaDexRouter: INSUFFICIENT_B_AMOUNT");
    ...
    ).to.be.revertedWith("DefiestaDexRouter: INSUFFICIENT_A_AMOUNT");
  ...
  context("When gas limit is too low to run the full transaction", function () {
    ...
    beforeEach("", async function () {
      ...
      const addLiquidityTx = await this.contracts.DefiestaDexRouter.addLiquidity(
        this.contracts.token0.address,
        this.contracts.token1.address,
        parseEther("1"),
        parseEther("4"),
    ...
    it("Call fail when the gasLimit is insufficient to fully run addLiquidity", async function () {
      ...
      /*                               Reload fixtures                              */
      /* -------------------------------------------------------------------------- */
      const { token0, token1, pair } = await loadFixture(unitFixtureDefiestaDexRouter);
