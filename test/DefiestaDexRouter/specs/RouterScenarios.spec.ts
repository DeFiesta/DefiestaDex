...
export function shouldBehaveLikeRouterScenarios() {
  ...
  it("should addLiquidity with MintCallback", async function () {
    ...
    expect(await this.contracts.DefiestaDexPair.balanceOf(this.signers.admin.address)).to.be.gt(0);
  });
  it("should addLiquidityETH with MintCallback", async function () {
    ...
    await this.contracts.token0.approve(this.contracts.DefiestaDexRouter.address, constants.MaxUint256);
    await this.contracts.DefiestaDexRouter.addLiquidityETH(
      this.contracts.token0.address,
      token0Amount,
      0,
      0,
    ...
    const pair = await this.contracts.DefiestaDexFactory.getPair(
      this.contracts.token0.address,
      this.contracts.WETH.address,
    );
    expect(await DefiestaDexPair__factory.connect(pair, this.signers.admin).balanceOf(this.signers.admin.address)).to.be.gt(
      0,
    );
  });
  ...
  it("addLiquidity and should swap Exact In", async function () {
    ...
    await this.contracts.token0
      ...
      .approve(this.contracts.DefiestaDexRouter.address, constants.MaxUint256);
    ...
    const fictiveReserves = await this.contracts.DefiestaDexPair.getFictiveReserves();
    const amountBOptimal = await this.contracts.DefiestaDexRouter.quote(
      TOKEN0_AMOUNT.add(1),
      fictiveReserves[0],
      fictiveReserves[1],
    );
    await expect(
      ...
      .withArgs(this.signers.admin.address, this.contracts.DefiestaDexPair.address, anyValue)
      .to.emit(this.contracts.token1, "Transfer")
      .withArgs(this.signers.admin.address, this.contracts.DefiestaDexPair.address, anyValue)
      .to.emit(this.contracts.DefiestaDexPair, "Transfer")
  ...
  it("addLiquidity and should swap Exact In to ETH", async function () {
    ...
    const WETHPartnerPair = await this.contracts.DefiestaDexFactory.getPair(
      this.contracts.WETH.address,
      this.contracts.WETHPartner.address,
    );
    ...
    const token1Pair = await this.contracts.DefiestaDexFactory.getPair(
      this.contracts.token1.address,
      this.contracts.WETHPartner.address,
    );
  ...
  it("addLiquidity and should swap Exact In with ETH with function swapExactETHForTokens", async function () {
    ...
    await this.contracts.DefiestaDexRouter.swapExactETHForTokens(
      0,
      firstPath,
      this.signers.user.address,
      constants.MaxUint256,
  ...
  it("addLiquidity and should swap ETH to Exact Out tokens with swapETHForExactTokens", async function () {
    ...
    const ethPair = await this.contracts.DefiestaDexFactory.getPair(
      this.contracts.WETH.address,
      this.contracts.WETHPartner.address,
    );
    ...
    const secondPath = [this.contracts.WETH.address, this.contracts.WETHPartner.address, this.contracts.token1.address];
    await this.contracts.DefiestaDexRouter.swapETHForExactTokens(
      parseEther("1"),
      secondPath,
      this.signers.user.address,
      constants.MaxUint256,
    ...
    await this.contracts.DefiestaDexRouter.swapTokensForExactETH(
      parseEther("1"),
      constants.MaxUint256,
      [this.contracts.token1.address, this.contracts.WETHPartner.address, this.contracts.WETH.address],
      this.signers.user.address,
  ...
  it("Try to destabilize pair with sending more tokens than expected", async function () {
    ...
    await this.contracts.token0
      ...
      .transfer(this.contracts.DefiestaDexPair.address, swapAmount.mul(100));
    ...
    await this.contracts.DefiestaDexPair.approve(this.contracts.DefiestaDexRouter.address, constants.MaxUint256);
    await this.contracts.DefiestaDexRouter.removeLiquidity(
      this.contracts.token0.address,
      this.contracts.token1.address,
      this.contracts.DefiestaDexPair.balanceOf(this.signers.admin.address),
      1,
      1,
      this.signers.admin.address,
      constants.MaxUint256,
