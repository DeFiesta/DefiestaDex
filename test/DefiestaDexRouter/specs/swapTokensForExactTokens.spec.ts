...
export function shouldBehaveLikeSwapTokensForExactTokens(): void {
  ...
  it("should revert with EXCESSIVE_INPUT_AMOUNT", async function () {
    await expect(
      this.contracts.DefiestaDexRouter.swapTokensForExactTokens(
        outputAmount.add(1),
        expectedTokenIn,
    ...
    ).to.be.revertedWith("DefiestaDexRouter: EXCESSIVE_INPUT_AMOUNT");
  });
  ...
  it("happy path", async function () {
    ...
    await expect(
      ...
      .withArgs(this.signers.admin.address, this.contracts.DefiestaDexPair.address, expectedTokenIn)
      .to.emit(this.contracts.token1, "Transfer")
      .withArgs(this.contracts.DefiestaDexPair.address, this.signers.admin.address, outputAmount)
  ...
  it("to of the swap can be router with address zero", async function () {
    ...
    ).to.be.revertedWith("DefiestaDexRouter: INVALID_RECIPIENT");

    // expect(await this.contracts.token1.balanceOf(this.contracts.DefiestaDexRouter.address)).to.be.eq(outputAmount);
  });
}
