...
export function shouldBehaveLikeUnwrapWETH(): void {
  it("should revert when insufficient WETH in balance", async function () {
    await expect(this.contracts.DefiestaDexRouterTest.unwrapWETHTest(1, this.signers.admin.address)).to.be.revertedWith(
      "Insufficient WETH",
    );
  });
  ...
  it("should not transfer any weth if balance is zero", async function () {
    ...
    await expect(this.contracts.DefiestaDexRouterTest.unwrapWETHTest(0, this.signers.admin.address)).to.not.be.reverted;
