import { ERC20Test, DefiestaDexRouter } from "../../typechain";
...
export interface MintTestData {
  ...
  expected_state_after_mint?: DefiestaDexPairStateTestData;
}

export interface DefiestaDexPairStateTestData {
  reserve0: BigNumber;
  reserve1: BigNumber;
...
export interface SwapTestData {
  ...
  to?: string;
  expected_revert_reason?: string;
  expected_state_after_swap?: DefiestaDexPairStateTestData;
}

export async function WETHPairInitialize(this: Context, WETHPartnerAmount: BigNumber, ETHAmount: BigNumber) {
  await this.contracts.WETHPartner.approve(this.contracts.DefiestaDexRouter.address, WETHPartnerAmount);
  await this.contracts.WETH.deposit({ value: ETHAmount });
  await this.contracts.WETH.approve(this.contracts.DefiestaDexRouter.address, ETHAmount);
