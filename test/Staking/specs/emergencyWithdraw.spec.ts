...
export function shouldBehaveLikeEmergencyWithdraw(): void {
  ...
  it("can't emergency withdraw multiple time", async function () {
    ...
    expect(await DDEX.balanceOf(admin.address)).to.eq(DefiestaDex_ADMIN_BALANCE.sub(AMOUNT_TOKENS_FOR_MINIMUM_SHARES));
    expect(await DDEX.balanceOf(staking.address)).to.eq(AMOUNT_TOKENS_FOR_MINIMUM_SHARES);
  ...
  it("more tokens in pool increase the amount of tokens to receive ", async function () {
    ...
    expect(await DDEX.balanceOf(user.address)).to.eq(
      DefiestaDex_USER_BALANCE.add(parseEther("90")).sub(AMOUNT_TOKENS_FOR_MINIMUM_SHARES),
    );
  });
  ...
  it("can emergency withdraw token to another address", async function () {
    ...
    expect(await DDEX.balanceOf(user.address)).to.eq(
      DefiestaDex_USER_BALANCE.add(parseEther("100")).sub(AMOUNT_TOKENS_FOR_MINIMUM_SHARES),
    );
  });
