...
import { MINIMUM_SHARES, parseShare, DefiestaDex_ADMIN_BALANCE, DefiestaDex_USER_BALANCE } from "../utils";
...
export function shouldBehaveLikeWithdraw(): void {
  ...
  it("can withdraw multiple time and still get the same exchange rate", async function () {
    ...
    expect(await DDEX.balanceOf(admin.address)).to.eq(DefiestaDex_ADMIN_BALANCE.sub(parseEther("100")));
    ...
    expect(await DDEX.balanceOf(admin.address)).to.eq(DefiestaDex_ADMIN_BALANCE.sub(parseEther("90")));
    ...
    expect(await DDEX.balanceOf(admin.address)).to.eq(DefiestaDex_ADMIN_BALANCE.sub(parseEther("40")));
  ...
  it("more tokens in pool increase the amount of tokens to receive ", async function () {
    ...
    expect(await DDEX.balanceOf(user.address)).to.eq(DefiestaDex_USER_BALANCE.sub(parseEther("100")).add(parseEther("10")));
    expect((await staking.userInfo(user.address)).shares).to.be.eq(parseShare(parseEther("90")).sub(MINIMUM_SHARES));

    await DDEX.transfer(staking.address, parseEther("90"));

    //should get 2x more token for the same withdraw
    await staking.connect(user).withdraw(user.address, parseShare(parseEther("10")));
    expect(await DDEX.balanceOf(user.address)).to.eq(DefiestaDex_USER_BALANCE.sub(parseEther("100")).add(parseEther("30")));
  });
  ...
  it("can withdraw token to another address", async function () {
    ...
    expect(await DDEX.balanceOf(user.address)).to.eq(DefiestaDex_USER_BALANCE.add(parseEther("10")));
  });
  ...
  it("check still tokens left after all users have withdrawn their shares ", async function () {
    ...
    await DDEX.transfer(staking.address, parseEther("1"));

    expect(await DDEX.balanceOf(admin.address)).to.be.eq(DefiestaDex_ADMIN_BALANCE.sub(parseEther("1")).sub(2));
    ...
    expect(await DDEX.balanceOf(admin.address)).to.be.eq(
      DefiestaDex_ADMIN_BALANCE.sub(parseEther("1")).add(parseEther("1").mul(sharesAdmin).div(totalShares)),
    );
    ...
    expect(await DDEX.balanceOf(user.address)).to.be.eq(
      DefiestaDex_USER_BALANCE.add(parseEther("1").mul(sharesUser).div(totalShares)),
    );
    expect(await DDEX.balanceOf(staking.address)).to.be.gt(0);
  ...
  it("max possible tokens in pool, deposit and withdraw ", async function () {
    ...
    await DDEX.connect(user).mint(user.address, MAX_POSSIBLE_AMOUNT);
    ...
    //  if the code wasn't able to handle 10 billion DDEX token with 18 decimals
    ...
    // add token
    await DDEX.transfer(staking.address, parseEther("10"));
    const stakingBalance = await DDEX.balanceOf(staking.address);
    ...
    expect(await DDEX.balanceOf(staking.address)).to.be.gt(constants.Zero);
  ...
  it("Audit test - Shares inflation", async function () {
    const attBalanceBefore = await DDEX.balanceOf(user.address);
    const victimBalanceBefore = await DDEX.balanceOf(admin.address);
    const depositAmount = parseEther("10");

    //console.log("***** BEFORE ATTACK *****");
    //console.log("Attacker token balance before deposit", attBalanceBefore.toString());
    //console.log("token balance of vault", (await DDEX.balanceOf(staking.address)).toString());

    //console.log("***** FIRST DEPOSIT - Attacker *****");
    await staking.connect(user).deposit(1);
    //console.log("Attacker shares", (await staking.userInfo(user.address)).shares);
    //console.log("Attacker token balance after deposit", await DDEX.balanceOf(user.address));
    ...
    //console.log("Attacker shares", (await staking.userInfo(user.address)).shares);
    //console.log("Attacker token balance after withdraw", await DDEX.balanceOf(user.address));
    //console.log("Total shares:", await staking.totalShares());

    //console.log("***** TRANSFER - Attacker *****");
    await DDEX.connect(user).transfer(staking.address, depositAmount);
    //console.log("Attacker shares", (await staking.userInfo(user.address)).shares);
    //console.log("Attacker token balance after deposit", await DDEX.balanceOf(user.address));
    //console.log("Total shares:", await staking.totalShares());
    ...
    const attBalanceAfter = await DDEX.balanceOf(user.address);
    ...
    expect(await DDEX.balanceOf(admin.address)).to.eq(victimBalanceBefore.sub(depositAmount));
