...
export function shouldBehaveLikeHarvestFarming(): void {
  ...
  it("should harvest DDEX in farming pool", async function () {
    await this.contracts.staking.harvestFarming();
    expect(await this.contracts.DefiestaDexTokenTest.balanceOf(this.contracts.staking.address)).to.be.eq(0);
    ...
    await this.contracts.staking.harvestFarming();
    expect(await this.contracts.DefiestaDexTokenTest.balanceOf(this.contracts.staking.address)).to.be.eq(parseEther("2"));
    ...
    await this.contracts.staking.harvestFarming();
    expect(await this.contracts.DefiestaDexTokenTest.balanceOf(this.contracts.staking.address)).to.be.eq(
