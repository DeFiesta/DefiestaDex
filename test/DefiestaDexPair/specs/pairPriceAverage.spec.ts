...
export function shouldBehaveLikeDefiestaDexPairPriceAverage(): void {
  context("Price average after add Liquidity", function () {
    it("initial price", async function () {
      const pAv = await this.contracts.DefiestaDexPair.getPriceAverage();
    ...
    it("Price after addLiquidity, same block", async function () {
      ...
      await this.contracts.token0.approve(this.contracts.DefiestaDexRouterTest.address, parseEther("100000"));
      await this.contracts.token1.approve(this.contracts.DefiestaDexRouterTest.address, parseEther("100000"));

      await this.contracts.DefiestaDexRouterTest.addLiquidity(
    ...
    it("Price after 1 big swap", async function () {
      ...
      //swap big
      await this.contracts.DefiestaDexRouterTest.swapExactTokensForTokens(
        parseEther("500"),
        0,
      ...
      let pAv = await this.contracts.DefiestaDexPair.getPriceAverage();
      ...
      const priceAvg = await this.contracts.DefiestaDexPair.getUpdatedPriceAverage(
      ...
      pAv = await this.contracts.DefiestaDexPair.getPriceAverage();
      const priceAvgAfter10min = await this.contracts.DefiestaDexPair.getUpdatedPriceAverage(
        fictiveReserves.fictiveReserve0_,
        fictiveReserves.fictiveReserve1_,
        pAv.priceAverageLastTimestamp_,
        pAv.priceAverage0_,
      ...
      const priceAvgAfter10minAndSwap = await this.contracts.DefiestaDexPair.getUpdatedPriceAverage(
        fictiveReserves.fictiveReserve0_,
        fictiveReserves.fictiveReserve1_,
        pAv.priceAverageLastTimestamp_,
        pAv.priceAverage0_,
    ...
    it("Price after 1 swap half reserve", async function () {
      await this.contracts.DefiestaDexPair.setFees(FEES_LP, FEES_POOL);
      //add liquidity to pair
      await this.contracts.token0.approve(this.contracts.DefiestaDexRouterTest.address, parseEther("100000"));
      await this.contracts.token1.approve(this.contracts.DefiestaDexRouterTest.address, parseEther("100000"));
