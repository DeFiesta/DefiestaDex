...
import { getAmount2TradesTestData, getAmountSimpleTestData, GetAmountTrade } from "../../DefiestaDexLibrary/utils";
...
export function shouldBehaveLikeSwapWithValues(): void {
  ...
  async function swapTest(
    ...
    if (isFeeToSet) {
      await contracts.DefiestaDexFactory.setFeeTo(signers.feeTo.address);
    }
  ...
  async function swap(contracts: Contracts, signers: Signers, isSwapToken0ToToken1: boolean, amountSpecified: string) {
    ...
    ).to.emit(contracts.DefiestaDexPairTest, "Swap");
  }
  ...
  async function checkResults(
    ...
    const priceAverageAfter = await contracts.DefiestaDexPairTest.getPriceAverage();
    ...
    const fictiveReservesAfter = await contracts.DefiestaDexPairTest.getFictiveReserves();
  ...
  async function checkFees(
    ...
    const fees = await contracts.DefiestaDexPairTest.getFeeToAmounts();
    const reserves = await contracts.DefiestaDexPairTest.getReserves();
    const balancePairToken0 = await contracts.token0.balanceOf(contracts.DefiestaDexPairTest.address);
    const balancePairToken1 = await contracts.token1.balanceOf(contracts.DefiestaDexPairTest.address);
