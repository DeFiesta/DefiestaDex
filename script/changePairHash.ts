...
export async function changePairHash(_args: undefined, _hre: HardhatRuntimeEnvironment) {
  const contractFactory = await _hre.ethers.getContractFactory("DefiestaDexPair");

  const DefiestaDexLibFile = __dirname + "/../contracts/periphery/libraries/PoolAddress.sol";
  console.log("🚜 -> file: changePairHash.ts:8 -> changePairHash -> PoolAddress", DefiestaDexLibFile);
  try {
    const data = await readFile(DefiestaDexLibFile, "utf8");
    ...
    await writeFile(DefiestaDexLibFile, result, "utf8");
  } catch (error) {
    console.error(error);
  }
}
