...
export const makePair = async (
  ...
  const tokenAbi = [
    ...(await hre.ethers.getContractAt("DefiestaDexTokenTest", token0)).interface.format("full"),
    {
      type: "function",
      name: "mint",
      constant: false,
  ...
  const DefiestaDexRouterAddress = await getDeployedRouterAddress(hre.network.name);
  const DefiestaDexRouter = await hre.ethers.getContractAt("DefiestaDexRouter", DefiestaDexRouterAddress);
  await addAllowance(token0Contract, DefiestaDexRouter, signer, waitingConfirmations);
  await addAllowance(token1Contract, DefiestaDexRouter, signer, waitingConfirmations);
  ...
  const addLiquidity = await DefiestaDexRouter.addLiquidity(
    token0,
    token1,
    values[0],
    values[1],
...
export const removeLiquidity = async (
  ...
  const DefiestaDexRouterAddress = await getDeployedRouterAddress(hre.network.name);
  const DefiestaDexRouter = await hre.ethers.getContractAt("DefiestaDexRouter", DefiestaDexRouterAddress);
  await addAllowance(pair, DefiestaDexRouter, signer, waitingConfirmations);
  ...
  // Now we add liquidity, router will take tokens
  const removeLiquidity = await DefiestaDexRouter.removeLiquidity(
    token0,
    token1,
    lp,
    0,
...
export const swapPair = async (
  ...
  const tokenAbi = [
    ...(await hre.ethers.getContractAt("DefiestaDexTokenTest", tokenin)).interface.format("full"),
    {
      type: "function",
      name: "mint",
      constant: false,
  ...
  const swapTx = await DefiestaDexRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
    amount,
    amountout || 1,
    [tokenin, tokenout],
    signer.address,
...
const addAllowance = async (
  ...
  DefiestaDexRouter: Contract,
  signer: SignerWithAddress,
  wait?: number,
) => {
  const allowance0 = await tokenContract.allowance(signer.address, DefiestaDexRouter.address);

  if (allowance0.lt(parseEther("1000000000000"))) {
    console.log("Increasing allowance...");
    const approve0 = await tokenContract.approve(DefiestaDexRouter.address, constants.MaxUint256);
...
export const getPair = async (
  ...
  const factory = await loadContract("DefiestaDexFactory", hre);
  ...
  const pair = await hre.ethers.getContractAt("DefiestaDexPair", pairAddress);
...
const getDeployedRouterAddress = async (networkName: string): Promise<string> => {
  const deploymentJsonFile =
    networkName !== "unknown" &&
    (await readFile(
      __dirname + "/../deployments/" + (networkName === "hardhat" ? "localhost" : networkName) + "/DefiestaDexRouter.json",
      { encoding: "utf-8" },
    ));
  const DefiestaDexRouterAddress = deploymentJsonFile ? JSON.parse(deploymentJsonFile).address : constants.AddressZero;
  console.log("Router Address : ", DefiestaDexRouterAddress);
  return DefiestaDexRouterAddress;
};
...
export const deployToken = async (
  ...
  const contractFactory = test
    ? await hre.ethers.getContractFactory("DefiestaDexTokenTest")
    : await hre.ethers.getContractFactory("DefiestaDexToken");
