...
import { ERC20Test, DefiestaDexRouter, WETH9 } from "../typechain";
...
export async function generateSwapGasCost(args: undefined, hre: HardhatRuntimeEnvironment) {
  ...
  const routerAddress = await getDeployedAddress(hre, "DefiestaDexRouter");
  const factoryAddress = await getDeployedAddress(hre, "DefiestaDexFactory");
  const wethAddress = await getDeployedAddress(hre, "WETH9");
  const router = await getContractAt("DefiestaDexRouter", routerAddress);
  const factory = await getContractAt("DefiestaDexFactory", factoryAddress);
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const pair = await getContractAt("DefiestaDexPairTest", pairAddress);
