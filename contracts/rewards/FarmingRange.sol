...
contract FarmingRange is IFarmingRange, Ownable, ReentrancyGuard {
    ...
    /// @inheritdoc IFarmingRange
    function addCampaignInfo(IERC20 _stakingToken, IERC20 _rewardToken, uint256 _startBlock)
        external
        virtual
        onlyOwner
    {
    ...
    /// @inheritdoc IFarmingRange
    function addRewardInfo(uint256 _campaignID, uint256 _endBlock, uint256 _rewardPerBlock)
        public
        virtual
        onlyOwner
        nonReentrant
    {
        ...
        require(
            rewardInfo.length < rewardInfoLimit, "FarmingRange::addRewardInfo::reward info length exceeds the limit"
        );
        require(
    ...
    /// @inheritdoc IFarmingRange
    function updateRewardInfo(uint256 _campaignID, uint256 _rewardIndex, uint256 _endBlock, uint256 _rewardPerBlock)
        public
        virtual
        onlyOwner
        nonReentrant
    {
        ...
        if (_rewardIndex != 0) {
            require(
                rewardInfo[_rewardIndex - 1].endBlock < _endBlock, "FarmingRange::updateRewardInfo::bad new endblock"
            );
        }
        ...
        (bool _refund, uint256 _diff) =
            _updateRewardsDiff(_rewardIndex, _endBlock, _rewardPerBlock, rewardInfo, campaign, selectedRewardInfo);
        if (!_refund && _diff != 0) {
    ...
    function updateCampaignsRewards(
        ...
        require(
            _campaignID.length == _rewardIndex.length && _rewardIndex.length == _endBlock.length
    ...
    function removeLastRewardInfo(uint256 _campaignID) external virtual onlyOwner {
        ...
        if (lastRewardInfo.rewardPerBlock != 0) {
            (bool _refund, uint256 _diff) = _updateRewardsDiff(
                _rewardInfoLength - 1, _lastRewardInfoEndBlock, 0, rewardInfo, campaign, lastRewardInfo
            );
            if (_refund) {
    ...
    function _transferFromWithAllowance(IERC20 _rewardToken, uint256 _amount, uint256 _campaignID) internal {
        try this.attemptTransfer(_rewardToken, rewardManager, address(this), _amount) {}
        catch {
     ...
     */
    function _pendingReward(uint256 _campaignID, uint256 _amount, uint256 _rewardDebt)
        internal
        view
        virtual
        returns (uint256)
        ...
        if (block.number > _campaign.lastRewardBlock && _campaign.totalStaked != 0) {
            ...
            for (uint256 _i; _i != _rewardInfo.length;) {
                ...
                if (_multiplier != 0) {
                    ...
                    _accRewardPerShare = _accRewardPerShare
    ...
    function _updateCampaign(uint256 _campaignID) internal virtual {
        ...
        for (uint256 _i; _i != _rewardInfo.length;) {
            ...
            if (_multiplier != 0) {
                ...
                campaign.accRewardPerShare = campaign.accRewardPerShare
