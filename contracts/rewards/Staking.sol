...
/**
 * @title Staking
 * @notice Implementation of an APY staking pool. Users can deposit DDEX for a share in the pool. New shares depend of
 * current shares supply and DDEX in the pool. Pool will receive DDEX rewards fees by external transfer from admin or
...
contract Staking is IStaking, ERC20 {
    ...
    constructor(IERC20 _DefiestaDexToken, IFarmingRange _farming) ERC20("Staked DefiestaDex Token", "stDDEX") {
        require(address(_DefiestaDexToken) != address(0), "Staking::constructor::DefiestaDex token is not defined");
    ...
    function deposit(uint256 _depositAmount) public isFarmingInitialized checkUserBlock {
        ...
        uint256 _currentBalance = DefiestaDexToken.balanceOf(address(this));
        ...
        DefiestaDexToken.safeTransferFrom(msg.sender, address(this), _depositAmount);
    ...
    function depositWithPermit(
        ...
        SafeERC20.safePermit(
            IERC20Permit(address(DefiestaDexToken)),
            msg.sender,
            address(this),
    ...
    // @audit Possible Cross-contract Reentrancy : CEI pattern  not implemented
    /// @inheritdoc IStaking
    function withdraw(address _to, uint256 _sharesAmount) external isFarmingInitialized checkUserBlock {
        ...
        // @audit-issue utilité d'appeler cette fonction?
        harvestFarming();
        ...
        DefiestaDexToken.safeTransfer(_to, _tokensToWithdraw);
    ...
    /**
     * @notice Calculate shares qty for an amount of DDEX tokens
     * @param _tokens user qty of DDEX to be converted to shares
     * @param _currentBalance contract balance DDEX. _tokens <= _currentBalance
    ...
    /**
     * @notice Calculate shares values in DDEX tokens
     * @param _shares amount of shares. _shares <= totalShares
     * @param _currentBalance contract balance in DDEX
     * @return tokens_ qty of DDEX token equivalent to the _shares. tokens_ <= _currentBalance
     */
