...
/**
 * @title AutoSwapper
 * @notice AutoSwapper makes it automatic and/or public to get fees from DefiestaDex and convert it to tokens for staking
 */
contract AutoSwapper is IAutoSwapper {
    ...
    constructor(IDefiestaDexFactory _factory, IERC20 _DefiestaDexToken, address _stakingAddress) {
    ...
    function transferTokens() public returns (uint256 _amount) {
        ...
        if (_amount != 0) {
            DefiestaDexToken.safeTransfer(stakingAddress, _amount);
        }
    }

    /**
     * @notice private function to swap token in DDEX and send it to the staking address
