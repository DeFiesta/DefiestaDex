...
import "./libraries/DefiestaDexLibraryV1.sol";
import "../../libraries/TransferHelper.sol";

// interfaces
import "./interfaces/IDefiestaDexPairV1.sol";
import "./interfaces/IDefiestaDexFactoryV1.sol";
import "./interfaces/IDefiestaDexSwapCallbackV1.sol";
 ...
 */
contract DefiestaDexPairV1 is IDefiestaDexPairV1, ERC20Permit {
    ...
    constructor() ERC20("DefiestaDex LP-Token", "SDEX-LP") ERC20Permit("DefiestaDex LP-Token") {
        factory = msg.sender;
    }

    ///@inheritdoc IDefiestaDexPairV1
    ...
    function getUpdatedPriceAverage(
        ...
        (priceAverageIn_, priceAverageOut_) = DefiestaDexLibraryV1.getUpdatedPriceAverage(
            _fictiveReserveIn,
            _fictiveReserveOut,
    ...
    function swap(
        ...
        (_params.priceAverageIn, _params.priceAverageOut) = DefiestaDexLibraryV1.getUpdatedPriceAverage(
            _params.fictiveReserveIn,
            _params.fictiveReserveOut,
        ...
        if (_amountSpecified > 0) {
            uint256 _temp;
            (_temp, , , _params.fictiveReserveIn, _params.fictiveReserveOut) = DefiestaDexLibraryV1.getAmountOut(
            ...
            uint256 _temp;
            (_temp, , , _params.fictiveReserveIn, _params.fictiveReserveOut) = DefiestaDexLibraryV1.getAmountIn(
                (-_amountSpecified).toUint256(),
                _params.balanceIn,
                _params.balanceOut,
                _params.fictiveReserveIn,
        ...
        if (_zeroForOne) {
            ...
            IDefiestaDexSwapCallbackV1(msg.sender).DefiestaDexSwapCallback(amount0_, amount1_, _data);
        ...
        // update feeTopart
        bool _feeOn = IDefiestaDexFactoryV1(factory).feeTo() != address(0);
        if (_zeroForOne) {
            if (_feeOn) {
                feeToAmount0 += ((uint256(amount0_) * DefiestaDexLibraryV1.FEES_POOL) / DefiestaDexLibraryV1.FEES_BASE)
                    .toUint104();
            }
            _update(
                _params.balanceIn,
            ...
            if (_feeOn) {
                feeToAmount1 += ((uint256(amount1_) * DefiestaDexLibraryV1.FEES_POOL) / DefiestaDexLibraryV1.FEES_BASE)
                    .toUint104();
            }
            _update(
                _params.balanceOut,
    ...
    function _mintFee() private returns (bool feeOn_) {
        address _feeTo = IDefiestaDexFactoryV1(factory).feeTo();
