...
// interfaces
import "./interfaces/IDefiestaDexFactory.sol";
import "./interfaces/IDefiestaDexPair.sol";
import "./interfaces/IDefiestaDexOwnableFactory.sol";
import "./interfaces/IDefiestaDexSwapCallback.sol";
import "./interfaces/IDefiestaDexMintCallback.sol";

/**
 * @title DefiestaDexPair
 * @notice Pair contract that allows user to swap 2 ERC20-strict tokens in a decentralised and automated way
 */
contract DefiestaDexPair is IDefiestaDexPair, ERC20Permit {
    ...
    modifier lock() {
        require(lockStatus == CONTRACT_UNLOCKED, "DefiestaDex: LOCKED");
    ...
    modifier onlyFactoryOwner() {
        require(msg.sender == IDefiestaDexOwnableFactory(factory).owner(), "DefiestaDex: NOT_OWNER");
        _;
    }

    constructor() ERC20("DefiestaDex LP-Token", "DDEX-LP") ERC20Permit("DefiestaDex LP-Token") {
        factory = msg.sender;
    }

    ///@inheritdoc IDefiestaDexPair
    function initialize(address _token0, address _token1, uint128 _feesLP, uint128 _feesPool) external override {
        require(msg.sender == factory, "DefiestaDex: FORBIDDEN"); // sufficient check
        token0 = _token0;
        token1 = _token1;
    ...
    function getUpdatedPriceAverage(
        ...
        (priceAverageIn_, priceAverageOut_) = DefiestaDexLibrary.getUpdatedPriceAverage(
            _fictiveReserveIn,
            _fictiveReserveOut,
    ...
    ///@inheritdoc IDefiestaDexPair
    function mint(address _to, uint256 _amount0, uint256 _amount1, address _payer)
        external
        override
    ...
    ///@inheritdoc IDefiestaDexPair
    function swap(address _to, bool _zeroForOne, int256 _amountSpecified, bytes calldata _data)
        external
        override
        lock
        returns (int256 amount0_, int256 amount1_)
    {
        require(_amountSpecified != 0, "DefiestaDex: ZERO_AMOUNT");

        IDefiestaDexPair.SwapParams memory _params = SwapParams({
            amountCalculated: 0,
            fictiveReserveIn: 0,
            fictiveReserveOut: 0,
            priceAverageIn: 0,
        ...
        require(_to != _params.token0 && _to != _params.token1, "DefiestaDex: INVALID_TO");
        (
            _params.balanceIn,
            _params.balanceOut,
        ...
        (_params.priceAverageIn, _params.priceAverageOut) = DefiestaDexLibrary.getUpdatedPriceAverage(
            _params.fictiveReserveIn,
            _params.fictiveReserveOut,
        ...
        if (_amountSpecified > 0) {
            uint256 _temp;
            (_temp,,, _params.fictiveReserveIn, _params.fictiveReserveOut) = DefiestaDexLibrary.getAmountOut(
            ...
            uint256 _temp;
            (_temp,,, _params.fictiveReserveIn, _params.fictiveReserveOut) = DefiestaDexLibrary.getAmountIn(
                DefiestaDexLibrary.GetAmountParameters({
                    amount: (-_amountSpecified).toUint256(),
                    reserveIn: _params.balanceIn,
                    reserveOut: _params.balanceOut,
        ...
        if (_zeroForOne) {
            ...
            IDefiestaDexSwapCallback(msg.sender).DefiestaDexSwapCallback(amount0_, amount1_, _data);
            ...
            require(
                ...
                "DefiestaDex: INSUFFICIENT_TOKEN0_INPUT_AMOUNT"
            );
            ...
            require(
                ...
                "DefiestaDex: INSUFFICIENT_TOKEN1_INPUT_AMOUNT"
            );
            _params.balanceOut = IERC20(token0).balanceOf(address(this));
        }

        // update feeTopart
        bool _feeOn = IDefiestaDexOwnableFactory(factory).feeTo() != address(0);
        if (_zeroForOne) {
            if (_feeOn) {
                feeToAmount0 += ((uint256(amount0_) * _feesPool) / DefiestaDexLibrary.FEES_BASE).toUint104();
            }
            _update(
                _params.balanceIn,
                _params.balanceOut,
            ...
            if (_feeOn) {
                feeToAmount1 += ((uint256(amount1_) * _feesPool) / DefiestaDexLibrary.FEES_BASE).toUint104();
            }
            _update(
                _params.balanceOut,
                _params.balanceIn,
    ...
    function _setFees(uint128 _feesLP, uint128 _feesPool) internal {
        require(_feesLP != 0, "DefiestaDex: ZERO_FEES_LP");
        require(_feesLP + _feesPool <= DefiestaDexLibrary.FEES_MAX, "DefiestaDex: FEES_MAX");
        feesLP = _feesLP;
        feesPool = _feesPool;
    ...
    function _update(
        ...
        require(_fictiveReserve0 != 0 && _fictiveReserve1 != 0, "DefiestaDex: FICTIVE_RESERVES_TOO_LOW");
        require(_fictiveReserve0 <= type(uint128).max && _fictiveReserve1 <= type(uint128).max, "DefiestaDex: OVERFLOW");
    ...
    function _sendFee() private returns (bool feeOn_) {
        address _feeTo = IDefiestaDexFactory(factory).feeTo();
     ...
     */
    function _mintBeforeFee(address _to, uint256 _amount0, uint256 _amount1, address _payer)
        internal
        lock
        returns (uint256 liquidity_)
    {
        ...
        IDefiestaDexMintCallback(msg.sender).DefiestaDexMintCallback(
            IDefiestaDexMintCallback.MintCallbackData({
                token0: token0,
                token1: token1,
                amount0: _amount0,
                amount1: _amount1,
        ...
        require(_balance0after >= _balance0 + _amount0, "DefiestaDex: INSUFFICIENT_AMOUNT_0");
        require(_balance1after >= _balance1 + _amount1, "DefiestaDex: INSUFFICIENT_AMOUNT_1");
        ...
        require(liquidity_ != 0, "DefiestaDex: INSUFFICIENT_LIQUIDITY_MINTED");
    ...
    function _burnBeforeFee(address _to) internal lock returns (uint256 amount0_, uint256 amount1_) {
        ...
        require(amount0_ != 0 && amount1_ != 0, "DefiestaDex: INSUFFICIENT_LIQUIDITY_BURNED");
    ...
    function _feeToSwap() internal {
        address _feeTo = IDefiestaDexOwnableFactory(factory).feeTo();
        ...
        // Will swap fees to DDEX and send them. We want this to not revert if the swap failed
