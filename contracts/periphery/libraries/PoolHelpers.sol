...
library PoolHelpers {
    ...
    function sortTokens(address _tokenA, address _tokenB) internal pure returns (address token0_, address token1_) {
        require(_tokenA != _tokenB, "DefiestaDexHelper: IDENTICAL_ADDRESSES");
        (token0_, token1_) = _tokenA < _tokenB ? (_tokenA, _tokenB) : (_tokenB, _tokenA);
        require(token0_ != address(0), "DefiestaDexHelper: ZERO_ADDRESS");
    }
    ...
    function getReserves(
        ...
        (uint256 _reserve0, uint256 _reserve1) = IDefiestaDexPair(PoolAddress.pairFor(_factory, _tokenA, _tokenB))
            .getReserves();
    ...
    function getFictiveReserves(
        ...
        (address _token0, ) = sortTokens(_tokenA, _tokenB);
        (uint256 _fictiveReserve0, uint256 _fictiveReserve1) = IDefiestaDexPair(
    ...
    function getPriceAverage(
        ...
        (address _token0, ) = sortTokens(_tokenA, _tokenB);
        (uint256 _priceAverage0, uint256 _priceAverage1, ) = IDefiestaDexPair(
            PoolAddress.pairFor(_factory, _tokenA, _tokenB)
        ).getPriceAverage();
    ...
    function quote(uint256 _amountA, uint256 _reserveA, uint256 _reserveB) internal pure returns (uint256 amountB_) {
        require(_amountA != 0, "DefiestaDexHelper: INSUFFICIENT_AMOUNT");
        require(_reserveA != 0 && _reserveB != 0, "DefiestaDexHelper: INSUFFICIENT_LIQUIDITY");
