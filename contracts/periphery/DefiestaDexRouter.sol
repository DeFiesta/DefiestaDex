...
// interfaces
import "../core/interfaces/IDefiestaDexFactory.sol";
import "../core/interfaces/IDefiestaDexPair.sol";
import "./interfaces/IDefiestaDexRouter.sol";
import "./interfaces/IWETH.sol";

/**
 * @title DefiestaDexRouter
 * @notice Router for execution of swaps and liquidity management on DefiestaDexPair
 */
contract DefiestaDexRouter is IDefiestaDexRouter {
    ...
    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, "DefiestaDexRouter: EXPIRED");
        _;
    }
    ...
    function DefiestaDexSwapCallback(int256 _amount0Delta, int256 _amount1Delta, bytes calldata _data)
        external
        override
    {
        ...
        require(msg.sender == PoolAddress.pairFor(factory, _tokenIn, _tokenOut), "DefiestaDexRouter: INVALID_PAIR");
    ...
    ///@inheritdoc IDefiestaDexMintCallback
    function DefiestaDexMintCallback(MintCallbackData calldata _data) external override {
        // ensure that msg.sender is a pair
        require(
            msg.sender == PoolAddress.pairFor(factory, _data.token0, _data.token1), "DefiestaDexRouter: INVALID_PAIR"
        );
        require(_data.amount0 != 0 || _data.amount1 != 0, "DefiestaDexRouter: Callback Invalid amount");

        pay(_data.token0, _data.payer, msg.sender, _data.amount0);
        pay(_data.token1, _data.payer, msg.sender, _data.amount1);
    }

    /// @inheritdoc IDefiestaDexRouter
    function addLiquidity(
        ...
        (amountA_, amountB_) =
            _addLiquidity(_tokenA, _tokenB, _amountADesired, _amountBDesired, _amountAMin, _amountBMin);
        address _pair = PoolAddress.pairFor(factory, _tokenA, _tokenB);
        bool _orderedPair = _tokenA < _tokenB;
        liquidity_ = IDefiestaDexPair(_pair).mint(
            _to, _orderedPair ? amountA_ : amountB_, _orderedPair ? amountB_ : amountA_, msg.sender
        );
    }
        ...
        returns (uint256 amountToken_, uint256 amountETH_, uint256 liquidity_)
    {
        ...
        liquidity_ = IDefiestaDexPair(_pair).mint(
            _to, _orderedPair ? amountToken_ : amountETH_, _orderedPair ? amountETH_ : amountToken_, msg.sender
        );
    ...
    function removeLiquidity(
        ...
        address _pair = PoolAddress.pairFor(factory, _tokenA, _tokenB);
        IDefiestaDexPair(_pair).transferFrom(msg.sender, _pair, _liquidity); // send liquidity to pair

        (uint256 _amount0, uint256 _amount1) = IDefiestaDexPair(_pair).burn(_to);
        ...
        require(amountA_ >= _amountAMin, "DefiestaDexRouter: INSUFFICIENT_A_AMOUNT");
        require(amountB_ >= _amountBMin, "DefiestaDexRouter: INSUFFICIENT_B_AMOUNT");
    }
    ...
    function removeLiquidityWithPermit(
        ...
        IDefiestaDexPair(_pair).permit(msg.sender, address(this), _value, _deadline, _v, _r, _s);
    ...
    function swapExactTokensForTokens(
        ...
        require(amountOut_ >= _amountOutMin, "DefiestaDexRouter: INSUFFICIENT_OUTPUT_AMOUNT");
    }
    ...
    function swapTokensForExactTokens(
        ...
        require(amountIn_ <= _amountInMax, "DefiestaDexRouter: EXCESSIVE_INPUT_AMOUNT");
    ...
    function swapTokensForExactETH(
        ...
        require(_path[_path.length - 1] == WETH, "DefiestaDexRouter: INVALID_PATH");
    ...
    {
        require(_path[0] == WETH, "DefiestaDexRouter: INVALID_PATH");
    ...
    function _swapExactOut(uint256 _amountOut, address _to, SwapCallbackData memory _data)
        ...
        returns (uint256 amountIn_)
    {
        require(_to != address(0), "DefiestaDexRouter: INVALID_RECIPIENT");

        (address _tokenOut, address _tokenIn) = _data.path.decodeFirstPool();
        bool _zeroForOne = _tokenIn < _tokenOut;

        // do the swap
        (int256 _amount0, int256 _amount1) = IDefiestaDexPair(PoolAddress.pairFor(factory, _tokenIn, _tokenOut)).swap(
            _to, _zeroForOne, -_amountOut.toInt256(), abi.encode(_data)
        );
    ...
    function _addLiquidity(
        ...
        if (IDefiestaDexFactory(factory).getPairs(_tokenA, _tokenB) == address(0)) {
            IDefiestaDexFactory(factory).createPair(_tokenA, _tokenB);
        }
        ...
        if (_reserveA == 0 && _reserveB == 0) {
            ...
            if (_amountBOptimal <= _amountBDesired) {
                require(_amountBOptimal >= _amountBMin, "DefiestaDexRouter: INSUFFICIENT_B_AMOUNT");
                ...
                require(_amountAOptimal >= _amountAMin, "DefiestaDexRouter: INSUFFICIENT_A_AMOUNT");
        ...
        returns (uint256 amountOut_)
    {
        ...
        bool _zeroForOne = _tokenIn < _tokenOut;
        (int256 _amount0, int256 _amount1) = IDefiestaDexPair(PoolAddress.pairFor(factory, _tokenIn, _tokenOut)).swap(
            _to, _zeroForOne, _amountIn.toInt256(), abi.encode(_data)
        );
    ...
    function quote(uint256 _amountA, uint256 _reserveA, uint256 _reserveB)
        ...
        virtual
        override
        returns (uint256 amountB_)
    {
        return PoolHelpers.quote(_amountA, _reserveA, _reserveB);
    }

    /// @inheritdoc IDefiestaDexRouter
    function getAmountOut(DefiestaDexLibrary.GetAmountParameters memory _param)
        external
        pure
        returns (
            uint256 amountOut_,
        ...
        )
    {
        ...
        (amountOut_, newReserveIn_, newReserveOut_, newFictiveReserveIn_, newFictiveReserveOut_) =
            DefiestaDexLibrary.getAmountOut(_param);
    }

    /// @inheritdoc IDefiestaDexRouter
    function getAmountIn(DefiestaDexLibrary.GetAmountParameters memory _param)
        external
        pure
        returns (
            uint256 amountIn_,
    ...
    {
        (amountIn_, newReserveIn_, newReserveOut_, newFictiveReserveIn_, newFictiveReserveOut_) =
            DefiestaDexLibrary.getAmountIn(_param);
    }

    /// @inheritdoc IDefiestaDexRouter
    function getAmountOutFromPair(uint256 _amountIn, address _tokenIn, address _tokenOut)
        external
        view
        returns (
            uint256 amountOut_,
        ...
        )
    {
        ...
        IDefiestaDexPair _pair = IDefiestaDexPair(IDefiestaDexFactory(factory).getPairs(_tokenIn, _tokenOut));
        require(address(_pair) != address(0), "DefiestaDexRouter: INVALID_TOKENS");
    ...
    /// @inheritdoc IDefiestaDexRouter
    function getAmountInFromPair(uint256 _amountOut, address _tokenIn, address _tokenOut)
        external
        view
        returns (
            uint256 amountIn_,
